...
int seconds = 3; // counter for keeping track of when a second elapses to reset rpm measurement
...
void loop()  { 
   ...
   //always drive motor each loop iteration with PMW
  ...
  if (millis() >= (3 * 1000)) {
    ...
    //evaluate rpm from recent data and make decisions
      ...
      analogWrite(motorPin,50);//get motor to decent speed to help it move
    }
}
